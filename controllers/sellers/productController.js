...
//DELETE Many {products with there ref}
export const deleteProductsWithRef = catchAsync(async (req, res, next) => {
  const { id } = req.params;

  // Find the category by ID to ensure it exists
  const deletedProdduct = await Product.findByIdAndDelete(id);
  ...
  const deletedOrders = await Order.deleteMany({ products: id });
  // Update cache
  const cacheKey = getCacheKey("Product", "", req.query);
  await redisClient.del(cacheKey);
  ...
  return res.status(204).json({
    status: "success",
    message: "Product and related products deleted successfully",
  });
});
...
export const updateProduct = catchAsync(async (req, res, next) => {
  ...
  if (attributes && attributes.length > 0) {
    ...
    finalPrice =
      price +
      attributePriceModifiers.reduce((total, modifier) => total + modifier, 0);
  }
