...
export const calculateSellerWallet = catchAsync(async (req, res, next) => {
    ...
    const vendor = await mongoose.model('Vendor').findById(sellerId);
    if (!vendor) {
    ...
    const collectedOrders = await Order.find({ vendorId: sellerId, cashCollected: true });
    ...
    if (!sellerWallet) {
        ...
        sellerWallet.totalDeliveryChargeEarned = totalDeliveryChargeEarned.toFixed(2);
