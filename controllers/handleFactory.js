...
export const updateOne = (Model) =>
  catchAsync(async (req, res, next) => {
    // Check and filter the fields to update
    let { allowedFields, filteredData } = checkFields(Model, req, next);

    // If the document contains a 'slug' field and 'name' is provided, create a slug
    if (allowedFields.includes("slug") && filteredData.name) {
      filteredData = {
        ...filteredData,
        slug: slugify(filteredData.name, { lower: true }),
    ...
    // Generate cache key for the specific document
    const cacheKeyOne = getCacheKey(Model.modelName, req.params.id);

    // Delete the previous cached document data
    await redisClient.del(cacheKeyOne);

    // Update the cache with the new document data
    await redisClient.setEx(cacheKeyOne, 3600, JSON.stringify(doc));

    // Delete the general cache for the list of documents (if query exists)
    const cacheKey = getCacheKey(Model.modelName, "", req.query);
    await redisClient.del(cacheKey);
