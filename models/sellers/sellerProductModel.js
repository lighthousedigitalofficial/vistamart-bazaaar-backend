...
sellerProductSchema.virtual('reviews', {
    ref: 'ProductReview',
    localField: '_id',
    foreignField: 'product',
})

sellerProductSchema.virtual('totalOrders', {
    ref: 'Order',
    localField: '_id',
...
sellerProductSchema.pre(/^find/, function (next) {
    this.populate({
        path: 'category',
        select: 'name',
    })
...
sellerProductSchema.post('findByIdAndDelete', async function (doc) {
    if (doc) {
        await mongoose.model('Review').deleteMany({ product: doc._id })
    }
})

const Product = sellerDbConnection.model('Product', sellerProductSchema)
